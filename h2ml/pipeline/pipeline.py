...
class PipelineConfig:
    ...
    Attributes:
        ...
        n_jobs:              Worker processes used to CV models in parallel (steps 1 and 3).
                             -1 uses all cores. Lower it for memory-heavy models: TabPFN
                             loads a torch checkpoint per worker and its device="auto"
                             claims every visible CUDA GPU, so a high n_jobs can exhaust
                             RAM or VRAM. Must not be 0. Default: -1.
