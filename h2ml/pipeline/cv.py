    ...
    def run(
        ...
        if _splitter is not None:
            ...
            splitter = self._build_splitter(
                step.task_type, X=X, y=y, coords=coords,
        ...
        for fold_id, (train_idx, test_idx) in enumerate(splitter.split(X, y)):
            ...
            try:
                ...
                fold_result = self._collect_fold(
                    fold_step, fold_id, X_train, X_test,
                    y_train_true, y_test_true, train_idx, test_idx, fit_time,
    ...
    def run_all(
        ...
        if _splitter is not None:
            ...
            prebuilt_splitter = self._build_splitter(
                steps[0].task_type, X=X, y=y, coords=coords,
                n_blocks_per_fold=n_blocks_per_fold,
                spatial_cv_method=spatial_cv_method,
                ahc_threshold=ahc_threshold,
                spatial_cv_metric=spatial_cv_metric,
        ...
        def _run_one(step: PredictorStep) -> CVResult:
            ...
            return self.run(
                step, X, y,
                best_params=params, y_true=y_true, inverse_fn=inverse_fn,
                coords=coords, n_blocks_per_fold=n_blocks_per_fold,
                spatial_cv_method=spatial_cv_method, ahc_threshold=ahc_threshold,
                spatial_cv_metric=spatial_cv_metric, pca_components=pca_components,
                exact_max_samples=exact_max_samples, knn_neighbors=knn_neighbors,
        ...
        return cast(list[CVResult], Parallel(n_jobs=n_jobs, backend='loky')(
