...
class TestGetShapValues:
    ...
    def test_multiclass_explicit_class_index_differs_from_default(
        ...
        class0_arr, _ = get_shap_values(
            fitted_rf_multiclass, multiclass_store, TaskType.CLASSIFICATION, class_index=0
        )
...
class TestSvcShapValues:
    ...
    def test_svc_probability_true_oof_shape(self, clf_store):
        ...
        svc_step = make_classifier(SVC(kernel="linear", probability=True, random_state=42),
                                   requires_scaling=True)
        shap_arr, importance = get_oof_shap_values(
            svc_step, clf_store, TaskType.CLASSIFICATION,
            n_splits=3, max_background=20,
        )
        assert shap_arr.shape == (N, len(FEATURE_NAMES))
        assert set(importance.index) == set(FEATURE_NAMES)
    ...
    def test_svc_rbf_oof_shape(self, clf_store):
        svc_step = make_classifier(SVC(kernel="rbf", probability=True, random_state=42),
                                   requires_scaling=True)
        shap_arr, _ = get_oof_shap_values(
            svc_step, clf_store, TaskType.CLASSIFICATION,
            n_splits=3, max_background=20,
    ...
    def test_svr_oof_shape(self, reg_store):
        ...
        shap_arr, importance = get_oof_shap_values(
            svr_step, reg_store, TaskType.REGRESSION,
            n_splits=3, max_background=20,
        )
        assert shap_arr.shape == (N, len(FEATURE_NAMES))
        assert set(importance.index) == set(FEATURE_NAMES)
